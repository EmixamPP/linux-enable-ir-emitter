...
from globals import UDEV_RULE_PATH, OPENRC_NAME, get_kernels, get_index
...
class Openrc:
    ...
    def disable() -> int:
        ...
        """
        exit_code = subprocess.run(["rc-update", "del", OPENRC_NAME, "default"], capture_output=True).returncode
        if exit_code:
    ...
    def enable(self) -> int:
        ...
        exit_code = subprocess.run(["rc-update", "add", OPENRC_NAME, "default"], capture_output=True).returncode
        exit_code = subprocess.run(["rc-service", OPENRC_NAME, "start"], capture_output=True).returncode
        if exit_code:
    ...
    def status() -> int:
        ...
        Returns:
            ...
            other value: error with the boot service
        """
        logging.info("Openrc not support status command")
        return 0
